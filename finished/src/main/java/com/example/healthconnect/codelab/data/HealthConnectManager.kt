...
class HealthConnectManager(private val context: Context) {
    ...
    suspend fun writeHeartRateSamples(start: ZonedDateTime, end: ZonedDateTime) {
        healthConnectClient.insertRecords(
            listOf(
                RestingHeartRateRecord(
                    time = start.toInstant(),
                    zoneOffset = start.offset,
                    beatsPerMinute = Random.nextLong(50, 150),
                )
            ) + buildHeartRateSeries(start, end)
        )
        Toast.makeText(context, "Heart record added", Toast.LENGTH_LONG).show()
    }
